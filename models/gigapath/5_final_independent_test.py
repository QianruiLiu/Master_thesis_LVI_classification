...
"""
This script performs the final independent test evaluation for the slide-level
GigaPath LVI classifier. The model is trained on the development set, using an
early-stopping validation subset for checkpoint selection and threshold
selection, and is then evaluated once on the fixed independent external test set.
Usage
-----
The example usage can be seen in run_tuning_scripts/independent_test_training.sh
...
def _deduplicate_records_by_slide(records: List[Dict]) -> List[Dict]:
    seen = set()
    out = []
    for r in records:
        sid = r.get("slide_id")
...
def select_confusion_heatmap_examples(
    records: List[Dict],
    category: str,
    rng: np.random.RandomState,
    random_n: int = 5,
) -> List[Dict]:
    """Select heatmap examples for one confusion class.

    Policy:
      - TP: include all TP slides with y_true=1, roi=1, and usable ROI PNG/meta.
      - FN: include all FN slides with y_true=1, roi=1, and usable ROI PNG/meta.
      - TN/FP: randomly sample up to random_n slides that have ROI PNG/meta assets.

    Returned records are deduplicated by slide_id.
    """

    cat_records = [r for r in records if r["confusion"] == category and r.get("roi_png_ok", False)]
    cat_records = _deduplicate_records_by_slide(cat_records)

    if category in ("TP", "FN"):
        cat_records = [
            r for r in cat_records
            if int(r.get("y_true", 0)) == 1 and int(r.get("roi", 0)) == 1
        ]
        cat_records.sort(key=lambda r: (-float(r["prob"]), r["slide_id"]))
        return cat_records

    if len(cat_records) <= int(random_n):
        return sorted(cat_records, key=lambda r: r["slide_id"])

    choose_idx = rng.choice(
        np.arange(len(cat_records), dtype=np.int64),
        size=int(random_n),
        replace=False,
    )
    picked = [cat_records[int(i)] for i in sorted(choose_idx.tolist())]
    picked.sort(key=lambda r: r["slide_id"])
    return picked
...
def generate_confusion_heatmaps_for_split(
    ...
    selected = {}
    selected_ordered = []
    rng_select = np.random.RandomState(args.seed + 20260409)
    tn_fn_random_n = int(getattr(args, "heatmap_random_n", 5))

    for cat in ["TP", "FP", "TN", "FN"]:
        picks = select_confusion_heatmap_examples(
            records=records,
            category=cat,
            rng=rng_select,
            random_n=tn_fn_random_n,
        )
        selected[cat] = picks
        if len(picks) == 0:
            print(f"[HEATMAP][SELECT] {cat}: no eligible slide with ROI PNG assets.")
        else:
            print(f"[HEATMAP][SELECT] {cat}: n={len(picks)}")
            for r in picks:
...
def plot_single_roc_curve(
    ...
    title: str = "Independent test ROC curve",
):
    import matplotlib.pyplot as plt
    ...
    if auc_value is not None and np.isfinite(auc_value):
        roc_label = f"ROC (AUC = {auc_value:.3f})"
    else:
        roc_label = "ROC"

    plt.figure(figsize=(6, 6), dpi=150)
    plt.plot(fpr, tpr, linewidth=2.0, label=roc_label)
    plt.plot([0.0, 1.0], [0.0, 1.0], linestyle="--", linewidth=1.0, alpha=0.7)
    plt.xlim(0.0, 1.0)
    plt.ylim(0.0, 1.0)
    plt.xlabel("1 - Specificity")
    ...
    return {
        "fpr": fpr.tolist(),
        "tpr": tpr.tolist(),
        "thresholds": np.asarray(roc_res["thresholds"], dtype=np.float32).tolist(),
    }
...
def build_items_from_tsv(labels_tsv: str, slides_dir: str, h5_root: str) -> List[SlideItem]:
    """Build SlideItem list by matching H5 <-> NDPI <-> labels from a specific TSV."""
...
# -----------------------------
# Training 
# This section trains one final model using the development training subset,
# selects the best checkpoint based on validation AUC, selects a threshold on
# the validation subset, and evaluates the selected model on the independent test set.
# -----------------------------
...
# -----------------------------
# Single split (your original behavior)
# -----------------------------

def run_final_external(args):
    set_seed(args.seed)

    develop_items = build_items_from_tsv(args.develop_tsv, args.slides_dir, args.h5_root)
    external_items = build_items_from_tsv(args.external_tsv, args.slides_dir, args.h5_root)

    y_dev = np.array([it.y for it in develop_items], dtype=np.int64)
    ...
    print(f"Final split: train={len(train_items)} val={len(val_items)} external_test={len(external_items)}")
    print(
        f"Train positives={sum(it.y for it in train_items)}  "
        f"Val positives={sum(it.y for it in val_items)}  "
        f"External positives={sum(it.y for it in external_items)}"
    )

    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    metrics = train_one_fold(args, train_items, val_items, external_items, out_dir)

    y_true = np.asarray(metrics.get("test_y_true", []), dtype=np.int64)
    y_prob = np.asarray(metrics.get("test_y_prob", []), dtype=np.float32)
    roc_res = compute_roc_points(y_true, y_prob)
    if roc_res is not None:
        external_auc = float(metrics.get("test_auc", float("nan")))
        roc_json = {
            "dataset": "external",
            "auc": external_auc,
            "fpr": np.asarray(roc_res["fpr"], dtype=np.float32).tolist(),
            "tpr": np.asarray(roc_res["tpr"], dtype=np.float32).tolist(),
            "thresholds": np.asarray(roc_res["thresholds"], dtype=np.float32).tolist(),
            "n_samples": int(y_true.size),
            "n_positive": int(np.sum(y_true == 1)),
            "n_negative": int(np.sum(y_true == 0)),
        }
        with open(out_dir / "external_roc.json", "w") as f:
            json.dump(roc_json, f, indent=2)
        plot_single_roc_curve(
            y_true=y_true,
            y_prob=y_prob,
            out_png=out_dir / "external_roc_curve.png",
            auc_value=external_auc,
            title="External test ROC curve",
        )
        metrics["external_roc_json"] = "external_roc.json"
        metrics["external_roc_png"] = "external_roc_curve.png"
        print(f"[ROC] saved {out_dir / 'external_roc.json'}")
        print(f"[ROC] saved {out_dir / 'external_roc_curve.png'}")
    else:
        print("[ROC] skipped external ROC export because y_true has <2 classes or predictions are invalid.")
...
def run_cv(args):
    ...
    items = build_items_from_tsv(args.develop_tsv, args.slides_dir, args.h5_root)
...
def build_argparser():
    ...
    ap.add_argument(
        ...
        type=str,
        required=True,
        help="Development-set TSV with columns: id ROI LVI (used for final train/val split)",
    )
    ap.add_argument(
        "--external_tsv",
        type=str,
        required=True,
        help="External-test TSV with columns: id ROI LVI (used only for final evaluation)",
    )
    ...
    # Final train/val split inside development set
    ap.add_argument("--val_frac", type=float, default=0.2, help="Fraction of development set reserved for validation (stratified)")

    # Optional internal CV on development set only
    ap.add_argument(
        "--cv_folds",
        type=int,
        default=0,
    ...
    ap.add_argument("--heatmap_block_px", type=int, default=8192,
    ...
    ap.add_argument("--heatmap_random_n", type=int, default=5, help="For TN/FN heatmaps, randomly sample up to this many eligible slides per class.")
    # Eval stability
